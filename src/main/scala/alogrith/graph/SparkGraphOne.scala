...
object SparkGraphOne {
  def main(args: Array[String]) {
    ...
    val tripRDD = graph.triplets.filter(t=>t.attr>5)
    for(triple <- tripRDD.collect){
      println(s"${triple.srcAttr._1} to ${triple.dstAttr._1}")
    }
    println
    ...
    def getMax(a:(VertexId,Int),b:(VertexId,Int)):(VertexId,Int)={
      if (a._2>b._2)a else b
    }
    println("max of outDegrees:" + graph.outDegrees.reduce(getMax) + " max of inDegrees:" + graph.inDegrees.reduce(getMax) + " max of Degrees:" + graph.degrees.reduce(getMax))
    println
    ...
    println("follower:"+oldestFollower)
